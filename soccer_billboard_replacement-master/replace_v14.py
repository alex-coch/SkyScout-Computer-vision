...
from vv_0807.util_v2 import static_point_detecor_homography, billboard_point_tracker_homography
...
if __name__ == "__main__":
    ...
    for j in range(len(image_lst)):
        ...
        if (j - base) > 1 and j < (len(image_lst) - 2):
            c1, d1, e1 = static_point_detecor_homography(image_lst[base], image_lst[j - 2])
            c2, d2, e2 = static_point_detecor_homography(image_lst[base], image_lst[j - 1])
            c3, d3, e3 = static_point_detecor_homography(image_lst[base], image_lst[j])
            c4, d4, e4 = static_point_detecor_homography(image_lst[base], image_lst[j + 1])
            c5, d5, e5 = static_point_detecor_homography(image_lst[base], image_lst[j + 2])
            d = (d1 + d2 + d3 + d4 + d5) / 5
            c = (c1 + c2 + c3 + c4 + c5) / 5
        ...
        for x in range(0, length):
            ...
            if x % int(kappa) == 0 and x + int(point[j]) + int(kappa) >= length and x + int(point[j])< length:
                ...
                # print(pts_dst)
                # print(max(a)-min(a))
                cropped_src = np.array([[min(a), min(b)], [((length - x) / int(kappa)) * max(a), min(b)],
                                        [((length - x) / int(kappa)) * max(a), max(b)], [min(a), max(b)]])
                # Calculate Homography
                h, status = cv2.findHomography(pts_src, pts_dst)
