...
from util_v2 import alignImages
...
if __name__ == "__main__":
    ...
    for j in range(len(image_lst)):
        ...
        if j>0:
            ...
            points1, points2 = alignImages(img0,img1)
            bil_points1 = []
            bil_points2 = []
            dist_points = []
            for ii in range(len(points1)):
                if min_list[int(points1[ii][0])]<points1[ii][1] and points1[ii][1] < max_list[int(points1[ii][0])]:
                    bil_points1.append(points1[ii][0])#, points1[ii][1]])
                    bil_points2.append(points2[ii][0])#, points2[ii][1]])
            ...
            if x % 100 == 0 and x + 99 < length:
                if x == 0:
                    if j > 0:
                        if dist_points[0] > 0:
                            pts_dst = np.array(
                                [[x, min_list[x]], [x + 99 - int(yp1), min_list[x + 99 - int(yp1)]],
                                 [x + 99 - int(yp1), max_list[x + 99 - int(yp1)]],
                                 [x, max_list[x]]])
                        else:
                            pts_dst = np.array(
                                [[x + int(yn1), min_list[x + int(yn1)]], [x + 99 + int(yn1), min_list[x + 99 + int(yn1)]],
                                 [x + 99 + int(yn1), max_list[x + 99 + int(yn1)]],
                                 [x + int(yn1), max_list[x + int(yn1)]]])
                            if int(yn1) > 100:
                                t1 = yn1 // 100
                                for t in range(int(t1)):
                                    pts_dst2 = np.array(
                                        [[x - (t+1) * 100 + int(yn1), min_list[x - (t+1) * 100 + int(yn1)]],
                                         [x - t * 100 + int(yn1), min_list[x - t * 100 + int(yn1)]],
                                         [x - t * 100 + int(yn1), max_list[x - t * 100 + int(yn1)]],
                                         [x - (t+1) * 100 + int(yn1), max_list[x - (t+1) * 100 + int(yn1)]]])
                                ...
                                if t2 > 0:
                                    pts_dst3 = np.array(
                                        [[x, min_list[x]],
                                         [x + int(t2+1), min_list[x + int(t2+1)]],
                                         [x + int(t2+1), max_list[x + int(t2+1)]],
                                         [x, max_list[x]]])
                        ...
                        if dist_points[0] > 0:
                            pts_dst = np.array(
                                [[x - 1 - int(yp1), min_list[x - 1 - int(yp1)]],
                                 [x + 99 - int(yp1), min_list[x + 99 - int(yp1)]],
                                 [x + 99 - int(yp1), max_list[x + 99 - int(yp1)]],
                                 [x - 1 - int(yp1), max_list[x - 1 - int(yp1)]]])
                        if dist_points[0] <= 0 and x + 99 + int(yn1) < len(min_list) - 1:
                            pts_dst = np.array(
                                [[x - 1 + int(yn1), min_list[x - 1 + int(yn1)]],
                                 [x + 99 + int(yn1), min_list[x + 99 + int(yn1)]],
                                 [x + 99 + int(yn1), max_list[x + 99 + int(yn1)]],
