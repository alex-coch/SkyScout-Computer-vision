...
from util_v2 import static_point_detector
...
if __name__ == "__main__":
    ...
    for j in range(len(image_lst)):
        ...
        # exit(0)
        for x in range(0, length):
            if x % 100 == 0 and x + int(point[j]) + 100 < length:
                pts_dst = np.array(
                    [[x + int(point[j]), min_list[x + int(point[j])]],
                     [x + int(point[j]) + 100, min_list[x + int(point[j]) + 100]],
                     [x + int(point[j]) + 100, max_list[x + int(point[j]) + 100]],
                ...
                # cv2.waitKey(0)
            if x % 100 == 0 and x + int(point[j]) + 100 >= length and x + int(point[j])< length:
                pts_dst = np.array(
                ...
                # cv2.imshow("test", im_dst)
                # cv2.waitKey(0)
            if x % 100 == 0 and int(point[j]) - 100 - x > 0:
                pts_dst2 = np.array(
                    [[int(point[j]) - 100 - x, min_list[int(point[j]) - 100 - x]],
                     [int(point[j]) - x, min_list[int(point[j]) - x]],
                     [int(point[j]) - x, max_list[int(point[j]) - x]],
                     [int(point[j]) - 100 - x, max_list[int(point[j]) - 100 - x]]])
            ...
            if x % 100 == 0 and int(point[j]) - 100 - x <= 0 and int(point[j]) - x>0:
                pts_dst2 = np.array(
                    [[0, min_list[0]],
                     [int(point[j]) - x, min_list[int(point[j]) - x]],
                     [int(point[j]) - x, max_list[int(point[j]) - x]],
