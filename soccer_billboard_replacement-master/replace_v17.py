...
from util_v2 import static_point_detecor_homography, billboard_point_tracker_homography, parameterization
...
if __name__ == "__main__":
    ...
    for j in range(len(image_lst)):
        ...
        # if (j - base) > 2 and j < (len(image_lst) - 3):
        #     c1, d1, e1 = static_point_detecor_homography(image_lst[base], image_lst[j - 3])
        #     c2, d2, e2 = static_point_detecor_homography(image_lst[base], image_lst[j - 2])
        #     c3, d3, e3 = static_point_detecor_homography(image_lst[base], image_lst[j - 1])
        #     c4, d4, e4 = static_point_detecor_homography(image_lst[base], image_lst[j])
        #     c5, d5, e5 = static_point_detecor_homography(image_lst[base], image_lst[j + 1])
        #     c6, d6, e6 = static_point_detecor_homography(image_lst[base], image_lst[j + 2])
        #     c7, d7, e7 = static_point_detecor_homography(image_lst[base], image_lst[j + 3])
        ...
        if j > 0:
            parameter = parameterization(j, image_lst, parameter, min_list)
        ...
        if j == 0:
            ...
            for i in range(0, length):
                if i % 100 == 0:
                    parameter.append(i)
        if j > 0:
            beta = parameter[0] - (parameter[1] - parameter[0])
            if beta > 0:
                parameter.insert(0, beta)
            gamma = parameter[-1] + (parameter[-1] - parameter[-2])
            if gamma < length:
                parameter.append(gamma)
        angle_list = []
        ...
        #     #     kappa = kappa + round(rr)
        #     #     rr = 0
        for x in range(len(parameter)-1):
            pts_dst = np.array(
                [[int(parameter[x]), min_list[int(parameter[x])]],
                 [int(parameter[x+1]), min_list[int(parameter[x+1])]],
                 [int(parameter[x+1]), max_list[int(parameter[x+1])]],
                 [int(parameter[x]), max_list[int(parameter[x])]]])
        ...
        if parameter[-1] < length:
            pts_dst = np.array(
                [[int(parameter[-1]), min_list[int(parameter[-1])]], [len(min_list) - 1, min_list[len(min_list) - 1]],
                 [len(min_list) - 1, max_list[len(min_list) - 1]], [int(parameter[-1]), max_list[int(parameter[-1])]]])
            # print(pts_dst)
            # print(max(a)-min(a))
            cropped_src = np.array(
                [[min(a), min(b)], [min(a) + ((length - (int(parameter[-1]))) / (int(parameter[-1] - parameter[-2]))) * (max(a) - min(a)), min(b)],
                 [min(a) + ((length - (int(parameter[-1]))) / int(parameter[-1] - parameter[-2])) * (max(a) - min(a)), max(b)],
                 [min(a), max(b)]])
        ...
        if parameter[0] > 0:
            pts_dst2 = np.array(
                [[0, min_list[0]],
                 [int(parameter[0]), min_list[int(parameter[0])]],
                 [int(parameter[0]), max_list[int(parameter[0])]],
                 [0, max_list[0]]])

            cropped_src2 = np.array(
                [[max(a) - (int(parameter[0]) / int(parameter[1]-parameter[0])) * (max(a) - min(a)), min(b)], [max(a), min(b)],
                 [max(a), max(b)], [max(a) - (int(parameter[0]) / int(parameter[1]-parameter[0])) * (max(a) - min(a)), max(b)]])
            # print(cropped_src2)
        ...
        # # Center coordinates
        #     center_coordinates = (int(parameter[m]), int(min_list[int(parameter[m])]))
        # #     # Make one pixel red
        #     im_dst = cv2.circle(im_dst, center_coordinates, radius, color, thickness)
